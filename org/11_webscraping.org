...
- The function ~str.join~ allows you to create suitable strings
...
* IN PROGRESS Project: opening all search results
...
- In ~' '.join('ab')~ works: ~' '~ is inserted between 'a' and 'b':
  #+begin_src python
    print('URL1' 'URL2' + ' '.join('webscraping'))
  #+end_src
...
- You need to open the page inspector and find the pattern that all
  ...
  ~'href'~ attribute. It helps that the inspector highlights the search
  result in the sidebar for every line of HTML/CSS code:
  #+attr_latex: :width 400px
  [[../img/11_pypi_bs4.png]]
...
- Add the code to the earlier block:
  #+name: soup
  #+begin_src python
    <<request>>
...
- A search without search term and with a nonsense search term:
  #+attr_latex: :width 400px
  [[../img/search_output1.png]]

- Here is the full program with exception handling for export:

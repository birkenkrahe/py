...
** Summary - what did we discuss today?
#+attr_latex: :width 300px
...
- Accessing items in lists that are list items, initializing empty list
...
2) How would you assign the value ~'hello'~ as the fourth value (index
   value = 3) in a list stored in a variable named ~spam = [2, 4, 6, 8,
   10]~?
   #+begin_src python
     spam = [2, 4, 6, 8, 10]
...
- Subsetting lists, checking if members ~in~ or ~not in~ a list
- Multiple variable assignments (tuple unpacking)
- Lists as arguments of functions from the ~random~ package:
  ~random.choice~ and ~random.shuffle~
- Augmented assignment operators for numbers, strings and lists
- Methods for ~lists~, ~np.array~ and ~pd.DataFrame~: find value with
  ~list.index~, ~np.where~, and ~pd.index~.
...
* DONE Day 15: Project day I (no class)
#+attr_latex: :width 400px
#+caption: USAF B-25 bomber above European patchwork landscape (1940)
[[../img/pattern.jpg]]

* DONE Day 16: Plotting practice
#+attr_latex: :width 400px
#+caption: Cottage loaves baked in an ancient oven at Llanrhaeadr-ym-Mochnant
[[../img/oven.jpg]]

** Test 3 is live - open until Sunday June 18

- *Plot challenge:* make a /density/ line plot of the test results of test
  1-2 and color the lines differently to make them stand out. Include
  the average in the diagram and print the name of the test next to
  each of the averages.

- Test results as lists:
  #+begin_src python :results silent
    test_1 = [18.17, 21, 21.5, 21.67, 23.17, 24]
    test_2 = [14.17, 17.67, 17.83, 19.17, 19.5, 23]
  #+end_src

- You have to have the ~seaborn~, ~matplotlib~ and ~numpy~ libraries
  installed and loaded to run this code:
  1) ~seaborn.kdeplot~ for the density smoothing
  2) ~matplotlib.pyplot.subplots~ for the 2 subplots
  3) ~numpy.mean~ to compute the average

- The complete code with plot title and axes labels:
  #+begin_src python
    ...
    sns.kdeplot(test_1, ax=ax, color='g')
    ax.axvline(np.mean(test_1), color='g', linestyle='--')
    ax.text(np.mean(test_1), 0.02, 'Test 1', color='g')

    sns.kdeplot(test_2, ax=ax, color='b')
    ax.axvline(np.mean(test_2), color='b', linestyle='--')
    ax.text(np.mean(test_2), 0.02, 'Test 2', color='b')

    ax.set_title('Test results CSC 109 Summer 2023')
    ax.set_xlabel('Available points')
...
- [[https://gist.github.com/birkenkrahe/ae9f9024a43a2379cc393e97907328ac][See solution documentation and code in GitHub.]]

** Test 4 will close on Friday, 23-June

- The test will include questions on the 2nd DataCamp course.

** Bonus deadline: 23-June for partial credit

- There's going to be one more regular and one more bonus assignment
...
- [[https://drive.google.com/drive/folders/1IEkIHzieBJ_tS5GPigsCxRZcioanlPof?usp=sharing][See also the whiteboard photos from the class session in GDrive]].
- Plotting practice (GitHub [[https://github.com/birkenkrahe/py/blob/main/org/plottingPractice.org][solution]] and [[https://gist.github.com/birkenkrahe/a61584bb88d5b97390e5781e9d62358c][practice file]])
- Making a histogram with ~matplotlib.pyplot.hist~
- Making a density estimate (smoothing curve) with [[https://seaborn.pydata.org/][seaborn]]
- Customizing histogram: title, labels, colors, grid, legend
- Barplots, lineplots, histograms; grammar of graphics and ggplot2
- Computing averages over array elements, adding them to a plot
- Formatting in an f-string with ~{var:.2f}~ and with ~round()~
- Color schemes in Python and [[https://matplotlib.org/stable/gallery/color/named_colors.html][in matplotlib]]
- Creating subplots with ~plt.subplots~, printing with ~plt.tight_layout~
- Window/panel setup with ~fig~ and ~axs~.
...
PLOTTING PRACTICE ([[https://lyon.instructure.com/courses/1700/assignments/15054][see Canvas]])

*Combine* the classroom exercises for histograms and subplots and put
the two customized histograms with averages of test 1 ([[https://github.com/birkenkrahe/py/blob/main/img/hist_avg_2.png][here]]) and test
2 ([[https://github.com/birkenkrahe/py/blob/main/img/hist_avg_3.png][here]]) in ONE graphic panel on top of one another like we did it for
a simple version of the histogram ([[https://github.com/birkenkrahe/py/blob/main/img/hist_1_2.png][here]]).

Also, make sure that the subplots share the same x-axis scale so that
you can compare them. /Tip:/ you can do that with this setup command:
#+begin_example python
  fig, axs = plt.subplot(2, sharex=True)
#+end_example

The final plot should look like this:
#+attr_html: :width 400px
[[../img/plottingExercise.png]]

*Submit* a link to your Colab notebook with the results in
[[https://lyon.instructure.com/courses/1700/assignments/15054][Canvas]]. Remember that you need to cite software, too. In the case of
Python, this includes Python 3 itself but also the packages used. The
package home pages usually tell you how they want to be cited, or you
can ask the web.
...
* DONE Day 17: Insert, append, remove, sort items
#+attr_latex: :width 400px
#+caption: Flanzaleys String Quartet (LOC@Flickr.com)
[[../img/strings.jpg]]

- See yesterday's homework: plotting practice!

** Preview & Review - lists and methods

1) What will this code print?
   #+begin_src python
     test = ['quick','brown','lazy']

     speed, color, kind, dog = test

     print(f'The {speed}, {color} fox jumped over the {kind} dog.')
   #+end_src

   #+RESULTS:

2) Is this loop allowed? What will happen?
   #+begin_src python
     from math import pi
     for i in [1, 'cat', pi, 'dog', True]:
         print(i)

     import numpy as np
     theOne = np.array([1,2,3])
     print(theOne)
     for i in theOne:
         print(i)
   #+end_src
...
3) What's the output of this code?
   #+begin_src python
     cat = 'jack'
     print(('cat', 'jack') in globals().items())
     from math import pi
     print(globals().items())
   #+end_src
   ...
   #+RESULTS:
   : True
   : dict_items([('__name__', '__main__'), ('__doc__', None), ('__package__', None), ('__loader__', <class '_frozen_importlib.BuiltinImporter'>), ('__spec__', None), ('__annotations__', {}), ('__builtins__', <module 'builtins' (built-in)>), ('__file__', '<stdin>'), ('__cached__', None), ('cat', 'jack'), ('pi', 3.141592653589793)])

4) Is there any difference between these programs or their output?
   #+begin_src python
     def names():
         names = ['jack','jill','joe','jane']
         return names
   ...
   #+begin_quote
   1) You define a function ~names()~ that returns a list of names.
   2) You then assign the ~return~ value of ~names()~ (which is a ~list~) to
      the variable ~names~.
   3) Now, ~names~ is no longer a function, but a ~list~.
   4) When you try to call ~names()~ on the next line, Python throws a
      ~TypeError~ because you're trying to call a list as if it were a
      function, which is not allowed.

   Fix: rename the function to ~getNames~ and don't try to call a
...
5) This code contains two mistakes. What are they and what's the fix?
   #+begin_src python
     import random
     pets = ['Jack', 'Nanny', 'Rosie', 'Poppy']
     pets = random.shuffle(pets)
     print(f'My favorite pets, in this order, are:\n{pets}')
   #+end_src

   #+RESULTS:
   : My favorite pets, in this order, are:
   : ['Jack', 'Rosie', 'Poppy', 'Nanny']

   #+begin_quote
   ~random.shuffle~ shuffles its argument in place and returns ~None~
   #+end_quote
...
- Methods for ~list~, NumPy's ~np.array~ and pandas' ~pd.DataFrame~ types:
...
MAGIC 8 BALL RELOADED (BONUS - SEE CANVAS)

1) You created a Magic 8 ball program as a fortune teller:
   #+begin_src python
     import random
...
2) Using lists, write a much more elegant version of the previous
   ...
   - You can also use ~random.choice~ to pick an item from ~messages~
...
3) Optionally, test the performance of the original Magic 8 Ball
   program vs. the new version in Colab using the ~%timeit~ magic
   function. [[https://colab.research.google.com/github/jakevdp/PythonDataScienceHandbook/blob/master/notebooks/01.07-Timing-and-Profiling.ipynb#scrollTo=ZJQ34W_23gdA][Here is some documentation on this and other functions.]]

* DONE Day 18: Sequence data, references, ~copy~
#+attr_latex: :width 400px
#+caption: Photo: Sam Hood / Schoolchildren line up for soup (1934)
[[../img/soup.jpg]]

** TODO [[https://lyon.instructure.com/courses/1700/assignments/14755][Last programming assignment "Student scores"]]
** TODO [[https://lyon.instructure.com/courses/1700/assignments/15059][Last bonus assignment: "Magic 8 Ball reloaded"]]
** TODO Summary
#+attr_latex: :width 300px
#+caption: Women spectators at wrestling, Sydney stadium 1940 (Flickr.com)
[[../img/summary.jpg]]
...
* DONE Day 19: Project Day II (no class)
#+attr_latex: :width 400px
#+caption: Photo: Sam Hood / Schoolchildren line up for soup (1934)
[[../img/spreadsheets.jpg]]

#+begin_quote
Change of plans for personal reasons:

We will not meet in class tomorrow, Friday June 16. Please work on
your project instead.  We will meet in class on Monday, June 19 at 1
pm to discuss Conway's Game of Life. The remaining sessions on Tuesday
and Wednesday will also contain applications as discussed. Thursday
and Friday is reserved for your projects (3 each per day - 30 min max
for presentation and discussion).  Thanks for your understanding!
#+end_quote


* DONE Day 20: Conway's Game of Life
#+attr_latex: :width 400px
...
If you got these questions wrong before I could fix it, you will did
the extra point!

*** Square brackets missing
In test 3 (at least) one question had an issue: square brackets
were missing in the array definition -
#+begin_src python :results output
  import numpy as np
  z = np.array([[1,1,1],
                [3,4,4]])
  print(z[1:,:1]) # on the console, prints `[[3]]`
  # while z[1][0] prints `3`
#+end_src
...
- Question not clear enough: ~except: ZeroDivisionError~ catches all
  errors, not just zero division, and there is the lacking
  indentation.

- Though you should have recognized the two errors anyway...
  
** TODO I must be doing something wrong
#+attr_latex: :width 400px
[[../img/fortune.png]]

** TODO Last test - new deadline Sunday 25 June!
#+attr_latex: :width 400px
#+caption: Ladies' Reliability Trials, Australia, 1936 (Flickr.com)
[[../img/ladies.jpg]]

Alas, I won't be finishing the test before middle of this week.

Test questions come mostly from DataCamp's "Intermediate Python":
- Matplotlib (line plot, scatter plot, histograms)
- Dictionaries and Pandas
- Flow control with NumPy and Pandas
- Loops over NumPy arrays and DataFrames
- NOT INCLUDED: Case Study ("Hacker Statistics")

(To create DataCamp test questions, I usually work through the
practice exercises on DataCamp and let myself be inspired.)

** TODO What does 'Let the data tell a story' mean?
#+attr_latex: :width 400px
[[../img/storytelling.png]]

- The second DataCamp course says "let the data tell a story", for
  example by customizing plots - what do you think about this?
  #+attr_latex: :width 400px
  [[../img/datacamp_storytelling.png]]

- What are the elements of story? Look at this short (1:20 min)
...
- What does this have to do with data?
  #+begin_quote
  * A priori: nothing. Structured data aren't story input
  * The digital data format is dominated by machine needs
  * The format of a narrative is dominated by human needs
  #+end_quote

- What does this have to do with Python?
  #+begin_quote
  * Programming reduces the storytelling toolbox drastically, limiting
    it to digital data structuring and visualization techniques
  * No neuroscientific or psychological basis that this works in the
    same way, or with similar efficiency as traditional storytelling
  #+end_quote

- MIT Sloan (and many others) offers 'Communicating data through
  storytelling' promising that the participants will walk away with:
  #+attr_latex: :width 400px
  [[../img/mit_storytelling.png]]

- Indicative: the MIT Sloan course curriculum
  #+attr_latex: :width 400px
  [[../img/mit_storytelling1.png]]

- What does this mean?
  #+begin_quote
  * 'Data storytelling' is a hype term not really like 'storytelling'
    (perhaps to appeal to people who don't like working with machines)
  * 'Machine learning' does the same thing to 'learning'
  * Leave storytelling to the experts (cp. 10,000 hour rule)
  * If you happen to like both machines and stories, accept that
    you're probably never going to master both at the same level
  #+end_quote
...
- Cellular automata: principle, practice and applications
- Conway's game of life with lists, dictionaries, NumPy, Matplotlib
- Using nested lists to simulate a grid
- Wrapping around a grid with floored division

* DONE Day 21: Web scraping I (~webbrowswer~, ~requests~)
#+attr_latex: :width 400px
...
* DONE Day 22: Web scraping II (~bs4~, ~selenium~)
#+attr_latex: :width 400px
#+caption: Group, Flickr.com
[[../img/group1.jpg]]

[[https://lyon.instructure.com/courses/1700/external_tools/163][Course evaluation link!]] (anonymous, by Sunday). I'm curious:

1) What did you think of the *DataCamp assignments* - more of it? Less?
   Should they be used more often in class? (I used them as an
   additional dimension instead.)
2) Was the number and level of *programming assignments* sufficient?
   Would you have liked more ? Different type (more difficult?
   Easier?)
3) What did you think about having *projects* in an introductory course
   (I've not done this before) - too much independence, too little
   guidance?
4) Remember what you said in the entrance survey: were your *hopes* for
   this course met with what was offered? (Or just your fears?)
5) Any comment on the *quizzes*? Too hard, too easy, too obscurely
   worded, would have wanted more/fewer?
6) Anything else that would help me improve the course, especially for
   *high school students* (they'll get this next, in fall)?

Thank you so much!

* DONE Day 23: 3 project presentations
#+attr_latex: :width 400px
...
- Ryan Isbell: Average Adventure Game ([[https://colab.research.google.com/drive/1eCcI3XkRK98qctwlMkK1iP0e2emrR6fD?usp=sharing][colab]])
- Lucia Birkenkrahe: Building a locally hosted website with Python ([[https://colab.research.google.com/drive/1CpIWEDXWOhyD5cSzpsqJqS7Q1RoPPBX3?usp=sharing][colab]])
- Dr. Radek Szulga: Scrape American Economic Review website ([[https://colab.research.google.com/drive/1eEZ0uiCQAZ9bpPVRvscsxWGSLDDiY4FZ?usp=sharing][colab]])


* Day 24: 3 project presentations/summary/outlook
#+attr_latex: :width 400px
...
- Clifford Washington: Hangman game
- Dr. Peter Staples: Data Scraping from Public Websites
- Jonathan Tucker: Stable Diffusion Training Using Textual Inversion
  with Webscraped Data Sets

** Summary / Outlook
#+attr_latex: :width 400px
[[../img/outlook.png]]

- From the entry survey: *what you wanted to learn*
  1) learn a language that is relevant to data science
  2) expand my capabilities as a programmer
  3) grasp programming concepts
  4) learn to script for network security applications

- Mostly missing bits: *what we didn't get into*
  1) text mining applications, natural language processing
  2) machine learning applications, neural nets
  3) visualization and animation, games
  4) interfacing with (non)relational databases

- Suggestions: *what to do next*
  1) Watch CS50-Python and code along in Colab
  2) Python programming: Eric Matthews, Python Crash Course
  3) Algorithms with Python: Tuckfield (2021)
  4) Data science: DataCamp courses and Vasiliev (2022)

- Miscellaneous: *what else comes to mind right now*
  1) Check out R for free: DataCamp or Matloff's [[https://github.com/matloff/fasteR]["fasteR" (2023)]].
  2) To use colab with R: [[https://colab.to/r][colab.to/r]] ([[https://colab.research.google.com/drive/16u37bAGQEhvAlMfwmSjmNdkCbKVp4zQc?usp=sharing][R Demo]])
  3) To keep using DataCamp after July 6: write to me for access.
  4) From novice to expert: write > 36,500 lines of code per year.
  5) AI coding assistants: use them carefully and eclectically.
  6) Check out a visual coding language: Snap! ([[https://snap.berkeley.edu][snap.berkeley.edu]])


* References

- GetSmarter (2023). Communicating Data Through Storytelling. Online
  Short Course, MIT Management Executive Eduction. URL:
  [[https://mit-online.getsmarter.com/presentations/lp/mit-sloan-communicating-data-through-storytelling-online-short-course/?cid=17123097342&utm_contentid=595893184926&ef_id=c:595893184926_d:c_n:g_ti:kwd-1524496353808_p:_k:data%20story%20telling%20course_m:p_a:143061320144&gad=1&gclid=Cj0KCQjw1rqkBhCTARIsAAHz7K3WA1w8ZCtN93AdO2qkMGvpUKrgvLGJ9-AFq6jvece6oY5yvrGOzVcaAtdQEALw_wcB&gclsrc=aw.ds][mit-online.getsmarter.com]].
- Matthews (2023). Python Crash Course (3e). NoStarch. URL:
  [[https://nostarch.com/python-crash-course-3rd-edition][nostarch.com]].
- Tuckfield (2021). Dive into Algorithms. NoStarch. URL: [[https://nostarch.com/search/dive%20into%20algorithm][nostarch.com]].
- Vasiliev (2022). Python for Data Science. NoStarch. URL:
  [[https://nostarch.com/python-data-science][nostarch.com]].

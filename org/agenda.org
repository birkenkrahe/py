...
3) How would you get the docstring for the method ~now~?
   #+begin_src python
     help(datetime.datetime.now)
   #+end_src
   
4) Now, travel back through time to the end of yesterday's session:
...
*** TODO Review ~if~, ~else~ and ~elif~ statements: spam or ham?
*** TODO Simply loopy: building a truth table

* Day 9: Importing modules and a game program
#+attr_latex: :width 400px

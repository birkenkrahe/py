...
1) What's the difference between an absolute and a relative path?
   ...
   origin of the file tree (e.g. ~C:\~), while the *relative* path
   specifies a location relative to your current directory (~PWD~).
   #+end_quote

2) You know 4 different Python shells already - which are they?
   #+begin_quote
   - The interactive Python shell in IDLE
   - The IPython code cell in Google Colab
...
** TODO DataCamp lessons / practice
...
** TODO Lecture/practice: Python Basics

...
* IN PROGRESS Practice using return values and statements
** DONE Calculate area of a rectangle
...
* TODO Practice Exception Handling
...
* TODO Practice Exception Handling (home/bonus)

- Write a function ~divide_numbers~ that asks the user to enter two
  numbers ~num1~ and ~num2~ and divides the first number by the second
  number.

- Inside the function, handle any potential exceptions that may occur
  during the division operation, such as division by zero or invalid
  input.

* TODO Short program: Zig-zag
